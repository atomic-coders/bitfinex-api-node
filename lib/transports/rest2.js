...
class RESTv2 {
   ...
   */
  currencies (cb = () => {}) {
    return this._makePublicRequest(`/conf/pub:map:currency:label`, cb, (data) => {
      const res = (data && data[0] && Array.isArray(data[0])) ? data[0] : data
      return this._doTransform(Currency, res)
    })
  }
   ...
   */
  orderHistory (symbol, start = null, end = null, limit = null, sort = null, cb) {
    const url = !_isEmpty(symbol)
   ...
   * @param {string} params.request
   * @param {string} params.nonce
   * @param {string} params.currency - The currency to look for.
   * @param {string} params.since - Return only the history after this timestamp. (Max 3 months before until)
   * @param {string} params.until - Return only the history before this timestamp. (Default now)
   * @param {int32} params.limit - Limit the number of entries to return.
   * @param {string} params.wallet - Return only entries that took place in this wallet. Accepted inputs are: “trading”, “exchange”, “deposit”.
   * @param {Method} cb
   * @return {Promise} p
   * @deprecated
   * @see https://docs.bitfinex.com/v1/reference#rest-auth-balance-history
   */
  balanceHistory (params, cb) {
      return this._makeAuthLegacyRequest('history', params, cb)
  }
