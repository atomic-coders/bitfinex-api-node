...
class WSv2 extends EventEmitter {
    ...
    return new Promise((resolve, reject) => {
      this.close().then(() => {
        this.open().then( () => {
          ...
          error => {
              debug(
                  'close->open action returned error.',
                  error.getMessage()
              )
              // try again
  ...
  _triggerPacketWD () {
    ...
    this.reconnect().catch(error => {
        debug(
            'recconect had a promise reject. Close and retry.',
            Date.now() - this._packetWDLastTS
        )
        // now try to close/cleanup and then retry a recconect
        this.close()
            .then(() => this.reconnect())
            .catch(closeError => {
              debug(
                'could not close, still try to reconnect.',
                Date.now() - this._packetWDLastTS)

              this.reconnect().catch(reconnectError2 => {
                    debug(
                        'could not reconnect 2, still try to reconnect.',
                        Date.now() - this._packetWDLastTS)
              })
        })
    })
